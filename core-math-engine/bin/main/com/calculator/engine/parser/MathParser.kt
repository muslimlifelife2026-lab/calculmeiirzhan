...
object MathParser {
    ...
    fun evaluateRPN(
        ...
        return evaluateRPNWithSteps(rpn, variables, precision).first
    }
            ...
            when (token) {
                ...
                is MathToken.Func -> {
                    ...
                    val result = when (token.name) {
                        "sin" -> {
                            val res = BigDecimalMath.sin(a, mc)
                            steps.add(MathStep("Вычисляем синус от ${formatVal(a)} радиан", "sin(${formatVal(a)}) = ${formatVal(res)}"))
                            res
                        }
                        "cos" -> {
                            val res = BigDecimalMath.cos(a, mc)
                            steps.add(MathStep("Вычисляем косинус от ${formatVal(a)} радиан", "cos(${formatVal(a)}) = ${formatVal(res)}"))
                            res
                        }
                        "tan" -> {
                            val res = BigDecimalMath.tan(a, mc)
                            steps.add(MathStep("Вычисляем тангенс от ${formatVal(a)} радиан", "tan(${formatVal(a)}) = ${formatVal(res)}"))
                            res
                        }
                        "asin" -> {
                            val res = BigDecimalMath.asin(a, mc)
                            steps.add(MathStep("Вычисляем арксинус от ${formatVal(a)}", "arcsin(${formatVal(a)}) = ${formatVal(res)}"))
                            res
                        }
                        "acos" -> {
                            val res = BigDecimalMath.acos(a, mc)
                            steps.add(MathStep("Вычисляем арккосинус от ${formatVal(a)}", "arccos(${formatVal(a)}) = ${formatVal(res)}"))
                            res
                        }
                        "atan" -> {
                            val res = BigDecimalMath.atan(a, mc)
                            steps.add(MathStep("Вычисляем арктангенс от ${formatVal(a)}", "arctan(${formatVal(a)}) = ${formatVal(res)}"))
                            res
                        }
                        "sinh" -> {
